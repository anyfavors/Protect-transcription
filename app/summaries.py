...
from app.http_clients import get_ollama_client
...
def get_summaries(period: str) -> dict:
    ...
    try:
        ...
        """)
        periods = [
            {"period_key": row["period_key"], "count": row["cnt"]} for row in cursor.fetchall()
        ]
...
async def generate_summary(period: str, period_key: str) -> dict:
    ...
    try:
        ...
        ollama_url = (settings.get("ollama_url") or OLLAMA_URL).rstrip("/")
        ollama_model = settings.get("ollama_model") or OLLAMA_MODEL

        system_prompt = (
            "You are a helpful home assistant summarising audio transcriptions captured by "
            "security cameras. The transcriptions may be in Danish, English, or a mix. "
            "Write your summary in the same language as the majority of the transcriptions. "
            "Be concise. Group related events. Note who spoke (by camera location) and any "
            "notable topics or visitors."
        )
        user_prompt = (
            f"Here are the transcriptions captured during {period_label}:\n\n"
            f"{transcript_block}\n\n"
            "Please write a clear, concise summary of what happened, organised by theme or "
            "time of day."
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=502, detail=f"Unexpected Ollama response: {exc}"
            ) from exc
